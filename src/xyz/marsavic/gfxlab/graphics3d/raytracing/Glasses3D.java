...
public class Glasses3D extends Raytracer {

	private final static double EPSILON = 1e-9;


	public Glasses3D(Scene scene, Camera camera) {
		super(scene, camera);
	}
	...
	protected Color sample(Ray ray) {
		Color r = Color.rgb(1.0, 0.1, 0.0);
		Color b = Color.rgb(0.0, 0.1, 1.0);

		Vec3 rP = ray.p();
		Vec3 rD = ray.d();

		double delta = 1.0 / 32.0;
		Ray rayR = Ray.pd(rP, Vec3.xp(rD.x() - delta, Vector.xy(rD.y(), rD.z())));
		Ray rayB = Ray.pd(rP, Vec3.xp(rD.x() + delta, Vector.xy(rD.y(), rD.z())));

		r = r.mul(sample(rayR, 32));
		b = b.mul(sample(rayB, 32));

		Color color = sample(ray, 32);
		color = color.add(r).add(b).div(3);
